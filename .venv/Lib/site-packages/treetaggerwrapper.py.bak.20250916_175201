...
.. warning:: Parameter files renaming.
   ...
   module maintenance (where, purpose, fundingâ€¦).
...
â€” see _`TreeTagger automatic locate` for details.
...
Example (with Python3, **Unicode strings** by default â€” with Python2 you
...
- **Guess TreeTagger location** â€” you can still provide :envvar:`TAGDIR` as
...
- **Text chunking** (tokenizing to provide treetagger input)
  ...
  creating the wrapper â€” which will replace internal chunking in the whole
  process.
...
and their specific features, you may try to chunk with default processingâ€¦
with no guaranty.
...
`Naming Files, Paths, and Namespaces`_ documentation â€” note: you cannot use ``/``
...
possibly followed by any char (ex. a space, a dashâ€¦), followed
...
8. For Posix (Linux, BSDâ€¦ MacOSX), in a list of standard directories:
...
.. note::
    ...
    space separators â€” a tab after the word, then spaces between items).
...
# See Unicode databaseâ€¦
ALONEMARKS = "!?Â¿;\"Â«Â»â€œâ€Â´`Â¨,*Â¤@Â°:%|Â¦/" \
             "()[\\]{}<>Â«Â»\u008b\u009b\u0093" \
             "&~=Â±Ã—\226\227" \
             "\t\n\r" \
             "\u2014\u203E\u0305\u2012\u2013" \
...
g_langsupport = {
    ...
    "fr": {
        ...
        "fclictic": "'-t-elles|-t-ils|-t-on|-ce|-elles|-ils|-je|-la|"
                    "-les|-leur|-lui|-mÃªmes|-memes|-mÃªme|-meme|-m'|-moi|-on|-toi|-tu|-t'|"
                    "-vous|-en|-y|-ci|-lÃ |-la",
        "number": NUMBER_EXPRESSION,
        "dummysentence": "Cela est une phrase inutile pour assurer la "
                         "transmission des donnÃ©es . .",
        "replurlexp": 'url-remplacÃ©e',
        "replemailexp": 'email-remplacÃ©',
        "replipexp": 'ip-remplacÃ©e>',
        "repldnsexp": 'dns-remplacÃ©'
    },
    "de": {
        "encoding": "utf-8",
...
# "C'est la fin ." (+google translateâ€¦) - in case someone tries to use
# the module for chunking an officially unsupport language.
g_langsupport['bg']['dummysentence'] = 'Ð¢Ð¾Ð²Ð° Ðµ ÐºÑ€Ð°Ñ . .'
g_langsupport['nl']['dummysentence'] = 'Dit is het einde . .'
g_langsupport['et']['dummysentence'] = 'See on lÃµpuks . .'
g_langsupport['fi']['dummysentence'] = 'TÃ¤mÃ¤ on loppu . .'
g_langsupport['gl']['dummysentence'] = 'Este Ã© o final . .'
g_langsupport['it']['dummysentence'] = 'Questa Ã¨ la fine . .'
g_langsupport['kr']['dummysentence'] = 'ë ì´ ì•¼ . .'
g_langsupport['la']['dummysentence'] = 'Hoc est finis . .'
g_langsupport['mn']['dummysentence'] = 'Ð­Ð½Ñ Ð½ÑŒ ÑÑ†ÑÐ¸Ð¹Ð½ Ð±Ð°Ð¹Ð½Ð° . .'
g_langsupport['pl']['dummysentence'] = 'To jest koniec . .'
g_langsupport['ru']['dummysentence'] = 'Ð­Ñ‚Ð¾ ÐºÐ¾Ð½ÐµÑ† . .'
...
class TreeTagger(object):
    ...
    def __init__(self, **kargs):
        ...
        :keyword CHUNKERPROC: function to call for chunking in place of
                            wrapper's chunking â€” default to None (use
                            standard chunking).
...
def locate_treetagger():
    ...
    *tagger*, containing :file:`lib` and :file:`bin` subdirectories) is search:
    ...
    # I initially listed some common home subdirsâ€¦ finally use
    ...
    # Any posix OS (including Linux, *BSDâ€¦ (so MacOSX), cygwin on Windows).
    if ON_POSIX:
        searchdirs.extend([
            '/usr/bin',
            '/usr/lib',
    ...
    # Will search for any *directory* matching tree â€¦ tagger â€¦
...
class TaggerPoll(object):
    ...
    Each of â€¦_asynch method returns a :class:`Job` object allowing to know if
    ...
    **Example of use**
    ...
    def _worker_main(self):
        while True:
            if DEBUG_MULTITHREAD:
                logger.debug("Worker waiting for job to pickâ€¦")
...
class Job(object):
    ...
    :ivar result: Final job processing result â€” or exception.
    """
