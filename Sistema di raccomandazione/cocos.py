...
if __name__ == '__main__' :
    ...
    # if a file name is provided, run on such file
    if len(sys.argv) >= 2:
        filename = sys.argv[1]
        ...
        if len(sys.argv) >= 3:
            ...
            max_attrs = cfg.MAX_ATTRS

        del sys.argv[1]

        cocos(filename, max_attrs)

    # otherwise, run on each file in the folder specified in the configuration file
    else:
